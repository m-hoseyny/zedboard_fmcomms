...
#define AUTOTB_TVOUT_out_vec  "../tv/cdatafile/c.stepGen.autotvout_out_vec.dat"

#define INTER_TCL  "../tv/cdatafile/ref.tcl"

// tvout file define: "out_vec"
#define AUTOTB_TVOUT_PC_out_vec  "../tv/rtldatafile/rtl.stepGen.autotvout_out_vec.dat"
...
#define stepGen AESL_ORIG_DUT_stepGen
extern void stepGen (
unsigned int out_vec[256]);
#undef stepGen
...
unsigned int out_vec[256])
{
	...
	if (wrapc_switch_file_token.good())
	{
		...
		if (atoi(AESL_num.c_str()) == AESL_transaction_pc)
		{
			...
			sc_bv<32> *out_vec_pc_buffer = new sc_bv<32>[256];
			int i = 0;
			...
			while (AESL_token != "[[/transaction]]")
			{
				...
				while (!no_x)
				{
					...
					if (x_found != string::npos)
					{
						if (!err)
						{
							cerr << "@W [SIM-201] RTL produces unknown value 'X' on port 'out_vec', possible cause: There are uninitialized variables in the C design." << endl;
							err = true;
						}
			...
			if (i > 0)
			{
				// RTL Name: out_vec
				{
					// bitslice(31, 0)
					// {
						// celement: out_vec(31, 0)
						// {
							sc_lv<32>* out_vec_lv0_0_255_1 = new sc_lv<32>[256];
						// }
					// }
					...
					// bitslice(31, 0)
					{
						...
						// celement: out_vec(31, 0)
						{
							...
							for (int i_0 = 0; i_0 <= 255; i_0 += 1)
							{
								...
								{
									out_vec_lv0_0_255_1[hls_map_index++].range(31, 0) = sc_bv<32>(out_vec_pc_buffer[hls_map_index].range(31, 0));
								}
							}
						}
					}
					...
					// bitslice(31, 0)
					{
						...
						// celement: out_vec(31, 0)
						{
							...
							for (int i_0 = 0; i_0 <= 255; i_0 += 1)
							{
								...
								// output_type_conversion : (out_vec_lv0_0_255_1[hls_map_index++]).to_uint64()
								if (&(out_vec[0]) != NULL) // check the null address if the c port is array or others
								{
									out_vec[i_0] = (out_vec_lv0_0_255_1[hls_map_index++]).to_uint64();
								}
							}
						}
					}
...
// [call_c_dut] ---------->
		...
		sc_bv<32>* out_vec_tvout_wrapc_buffer = new sc_bv<32>[256];
			...
			// bitslice(31, 0)
			{
				...
				// celement: out_vec(31, 0)
				{
					...
					for (int i_0 = 0; i_0 <= 255; i_0 += 1)
					{
						...
						if (&(out_vec[0]) != NULL) // check the null address if the c port is array or others
						{
							sc_lv<32> out_vec_tmp_mem;
							out_vec_tmp_mem = out_vec[i_0];
							out_vec_tvout_wrapc_buffer[hls_map_index++].range(31, 0) = out_vec_tmp_mem.range(31, 0);
						}
					}
				}
			}
		...
		tcl_file.set_num(256, &tcl_file.out_vec_depth);
